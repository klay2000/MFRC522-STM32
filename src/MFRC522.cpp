 ...
 * Constructor.
 */
MFRC522::MFRC522() : MFRC522(SS, UINT8_MAX, 1)
...
MFRC522::MFRC522(byte resetPowerDownPin ///< Arduino pin connected to MFRC522's reset and power down input (Pin 6, NRSTPD, active low). If there is no connection from the CPU to NRSTPD, set this to UINT8_MAX. In this case, only soft reset will be used in PCD_Init().
				 ) : MFRC522(SS, resetPowerDownPin, 1)
...
MFRC522::MFRC522(byte chipSelectPin,	 ///< Arduino pin connected to MFRC522's SPI slave select input (Pin 24, NSS, active low)
				 ...
				 int port				 ///< SPI port on Arduino to use
)
{
	_chipSelectPin = chipSelectPin;
	_resetPowerDownPin = resetPowerDownPin;
	SPIObject = new SPIClass(port);
} // End constructor
...
{
	SPIObject->beginTransaction(SPISettings(MFRC522_SPICLOCK, MSBFIRST, SPI_MODE0)); // Set the settings to work with SPI bus
	digitalWrite(_chipSelectPin, LOW);												 // Select slave
	SPIObject->transfer(reg);														 // MSB == 0 is for writing. LSB is not used in address. Datasheet section 8.1.2.3.
	SPIObject->transfer(value);
	digitalWrite(_chipSelectPin, HIGH); // Release slave again
	SPIObject->endTransaction();		// Stop using the SPI bus
...
)
{
	...
	{
		SPIObject->transfer(values[index]);
	}
	digitalWrite(_chipSelectPin, HIGH); // Release slave again
	SPIObject->endTransaction();		// Stop using the SPI bus
} // End PCD_WriteRegister()
...
)
{
	...
	SPIObject->beginTransaction(SPISettings(MFRC522_SPICLOCK, MSBFIRST, SPI_MODE0)); // Set the settings to work with SPI bus
	digitalWrite(_chipSelectPin, LOW);												 // Select slave
	SPIObject->transfer(0x80 | reg);												 // MSB == 1 is for reading. LSB is not used in address. Datasheet section 8.1.2.3.
	value = SPIObject->transfer(0);													 // Read the value back. Send 0 to stop reading.
	digitalWrite(_chipSelectPin, HIGH);												 // Release slave again
	SPIObject->endTransaction();													 // Stop using the SPI bus
	return value;
...
)
{
	...
	SPIObject->transfer(address);													 // Tell MFRC522 which address we want to read
	if (rxAlign)
	{ // Only update bit positions rxAlign..7 in values[0]
		// Create bit mask for bit positions rxAlign..7
		byte mask = (0xFF << rxAlign) & 0xFF;
		// Read value and tell that we want to read the same address again.
		byte value = SPIObject->transfer(address);
	...
	while (index < count)
	{
		values[index] = SPIObject->transfer(address); // Read value and tell that we want to read the same address again.
		index++;
	}
	values[index] = SPIObject->transfer(0); // Read the final byte. Send 0 to stop reading.
	digitalWrite(_chipSelectPin, HIGH);		// Release slave again
	SPIObject->endTransaction();			// Stop using the SPI bus
} // End PCD_ReadRegister()
